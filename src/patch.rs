...
pub fn patch<S, W>(old_content: &[u8], delta: Delta<S::HashType>, out: &mut W)
where
    S: StrongHash,
    W: Write,
{
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            PatchError::ChunkOutOfBound {
                ...
                write!(
                    f,
                    "chunk out of bound for chunk {}: {} {}",
                    chunk_num, chunk_size, old_content_len
                )
            }
            PatchError::ChunkHashMismatch {
                chunk_num,
                ...
                write!(
                    ...
                    chunk_num, old_content_hash, new_hash
                )
            }
